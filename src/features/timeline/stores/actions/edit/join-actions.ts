...
import { canMutateTimelineItems, isLinkedSelectionEnabled } from './shared'
...
export function joinItems(itemIds: string[]): void {
  ...
  const mutationIds = groupDescriptors.flatMap((group) => group.itemIds)
  if (groupDescriptors.length === 0 || !canMutateTimelineItems(mutationIds)) return
