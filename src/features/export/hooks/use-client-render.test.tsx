...
vi.mock('@/features/export/deps/media-library', () => ({ resolveMediaUrls: mocks.resolveMediaUrls }))
vi.mock('../utils/smart-copy', () => ({ trySmartCopyExport: mocks.trySmartCopyExport }))
vi.mock('../utils/render-pipeline', () => ({
  isExtendedSettings: (settings: unknown) => typeof settings === 'object' && settings !== null && 'mode' in settings,
...
vi.mock('@/features/export/deps/timeline', () => ({
  useTimelineStore: { getState: () => ({ tracks: [], items: [], transitions: [], fps: 30, inPoint: null, outPoint: null, keyframes: [], busAudioEq: [], masterBusDb: 0, backgroundColor: '#000', width: 1920, height: 1080 }) },
}))
...
vi.mock('@/shared/projects/defaults', () => ({ DEFAULT_PROJECT_WIDTH: 1920, DEFAULT_PROJECT_HEIGHT: 1080 }))
...
const settings = { quality: 'medium', resolution: { width: 640, height: 360 } } as Record<string, unknown>
const renderedResult = {
  blob: new Blob(['encoded']), fileSize: 7, duration: 1, mimeType: 'video/mp4',
}
...
beforeEach(() => {
  ...
  const clientSettings = { resolution: { width: 640, height: 360 }, subtitleMode: 'burn', codec: 'avc', container: 'mp4' }
  mocks.mapRequestedClientSettings.mockReturnValue({ clientSettings, exportMode: 'video', renderWholeProject: false })
  mocks.resolveClientSettings.mockImplementation(async (settings: { subtitleMode?: string }) => ({
    ...
    exportMode: 'video', renderWholeProject: false,
  }))
...
describe('useClientRender lifecycle ownership', () => {
  it('aborts the active render on unmount and propagates its signal through media resolution', async () => {
    ...
    mocks.runRender.mockReturnValue(render.promise.then((result) => ({ result, renderPath: 'worker' })))
    const hook = renderHook(() => useClientRender())

    let exportPromise!: Promise<void>
    await act(async () => { exportPromise = hook.result.current.startExport(settings as never); await Promise.resolve() })
    expect(mocks.resolveMediaUrls).toHaveBeenCalledWith([], expect.objectContaining({ useProxy: false, signal: expect.any(AbortSignal) }))
  ...
  it('aborts replacement renders and releases a result that becomes stale', async () => {
    ...
    mocks.runRender.mockReturnValueOnce(first.promise.then((result) => ({ result, renderPath: 'worker' })))
      .mockReturnValueOnce(second.promise.then((result) => ({ result, renderPath: 'worker' })))
    const hook = renderHook(() => useClientRender())
    let firstExport!: Promise<void>
    await act(async () => { firstExport = hook.result.current.startExport(settings as never); await Promise.resolve() })
    const firstSignal = mocks.runRender.mock.calls[0]![0].signal as AbortSignal
    let secondExport!: Promise<void>
    await act(async () => { secondExport = hook.result.current.startExport(settings as never); await Promise.resolve() })
    expect(firstSignal.aborted).toBe(true)
    first.resolve(renderedResult)
    second.resolve({ ...renderedResult, blob: new Blob(['second']) })
    await act(async () => { await Promise.all([firstExport, secondExport]) })
  ...
  it('releases a rendered output when finalization/update work throws before ownership transfer', async () => {
    const output = { ...renderedResult, temporaryOutput: { directory: 'scratch', fileName: 'out.mp4' } }
    mocks.runRender.mockResolvedValue({ result: output, renderPath: 'worker' })
    mocks.buildTranscriptSubtitleCues.mockImplementation(() => { throw new Error('state/update failed') })
    const hook = renderHook(() => useClientRender())
    await act(async () => { await hook.result.current.startExport({ ...settings, subtitleMode: 'sidecar' } as never) })
  ...
  it('releases a successfully owned result once on unmount, with reset/unmount causing no double release', async () => {
    ...
    const hook = renderHook(() => useClientRender())
    await act(async () => { await hook.result.current.startExport(settings as never) })
